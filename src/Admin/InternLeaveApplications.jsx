// Internleaveapplication.jsx
import React, { useState, useEffect } from "react";
import axios from "axios";
import CustomNavbar from "./CustomNavbar";
import useTitle from "@/Components/useTitle";

const Internleaveapplication = ({ mode = "hr" }) => {
  ...
  const getLeavesEndpoint = () => {
    // HR mode -> fetch only interns of that HR
    // Admin mode -> fetch all leaves
    return mode === "hr"
      ? `${import.meta.env.VITE_BASE_URL}/leave/hr`
  ...
  useEffect(() => {
    ...
    const fetchLeaves = async () => {
      setLoading(true);
      try {
        const response = await axios.get(getLeavesEndpoint(), {
          // If you use cookies/session auth:
          withCredentials: true,
          // If you're using Bearer tokens instead, remove withCredentials and add:
          // headers: { Authorization: `Bearer ${token}` }
        });

        if (!cancelled) {
          // backend expected to return { leaves: [...] }
          const leavesFromServer = response.data?.leaves ?? [];
          setLeaves(leavesFromServer);
        }
      } catch (err) {
        ...
        if (!cancelled) {
          setMessage({
            text:
              ...
              "Failed to fetch leaves. Check console for details.",
            type: "error",
          });
          setLeaves([]);
        }
  ...
  const handleUpdateStatus = async () => {
    if (!selectedLeave || !status || !adminName.trim()) {
      setMessage({
  ...
  const countByStatus = (s) => leaves.filter((l) => l.status === s).length;
      ...
      <div className="p-6 bg-gray-50 min-h-screen">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-3xl font-bold text-gray-800 mb-6 text-center">
            {mode === "hr" ? "HR - Leave Dashboard" : "Admin - Leave Dashboard"}
          </h1>
              ...
              <div className="bg-white rounded-nonexl shadow-md overflow-hidden">
                <div className="bg-yellow-100 px-6 py-3 border-b border-yellow-200">
                  <h2 className="text-lg font-semibold text-yellow-800 flex items-center">
                    ...
                    Pending Applications ({countByStatus("Pending")})
                  </h2>
                </div>
              ...
              <div className="bg-white rounded-nonexl shadow-md overflow-hidden">
                <div className="bg-green-100 px-6 py-3 border-b border-green-200">
                  <h2 className="text-lg font-semibold text-green-800 flex items-center">
                    ...
                    Approved Leaves ({countByStatus("Approved")})
                  </h2>
                </div>
                <div className="divide-y divide-gray-200">
                  {leaves
              ...
              <div className="bg-white rounded-nonexl shadow-md overflow-hidden">
                <div className="bg-red-100 px-6 py-3 border-b border-red-200">
                  <h2 className="text-lg font-semibold text-red-800 flex items-center">
                    ...
                    Rejected Leaves ({countByStatus("Rejected")})
                  </h2>
                </div>
                <div className="divide-y divide-gray-200">
                  {leaves
                ...
                <div className="space-y-4">
                  <div>
                    <p className="text-gray-700">
                      ...
                      {selectedLeave.internid?.name || "Deleted user"}
                    </p>
                    <p className="text-gray-700">
