...
const UserManagement = () => {
  ...
  const filteredVerifiedUsers = useMemo(() => {
    return verifiedUsers.filter((user) => {
      ...
      const status = endDate && today > endDate ? "inactive" : "active";

      const matchesStatus = filterStatus === "all" || status === filterStatus;

      return matchesSearch && matchesRole && matchesStatus;
    });
  }, [verifiedUsers, searchTerm, filterRole, filterStatus]);
  ...
  const paginatedVerifiedUsers = useMemo(() => {
    const start = (verifiedPage - 1) * rowsPerPage;
    return filteredVerifiedUsers.slice(start, start + rowsPerPage);
  }, [filteredVerifiedUsers, verifiedPage, rowsPerPage]);

  const paginatedUnverifiedUsers = useMemo(() => {
    const start = (currentPage - 1) * rowsPerPage;
    return filteredUnverifiedUsers.slice(start, start + rowsPerPage);
  }, [filteredUnverifiedUsers, currentPage, rowsPerPage]);
  ...
  const handleAcceptUser = async (userId) => {
    try {
      ...
      GetAllUser();
      toast.info("User verified successfully!");
    } catch (error) {
      console.error("Error accepting user:", error);
      toast.info("Failed to verify user");
    }
  };
  ...
  const handleUpdateUser = async () => {
    ...
    if (
      ...
      selectedUser.role !== "hrHead"
    ) {
      toast("You cannot remove your own admin access");
      return;
    }
    setIsLoading(true);
    try {
      ...
      toast.info("Failed to update user");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDeleteUser = async (userId) => {
    if (window.confirm("Are you sure you want to delete this user?")) {
      try {
        ...
        toast.info("Failed to delete user");
      }
    }
  };

  const promoteUser = async (userId, newRole) => {
    if (!window.confirm(`Promote this user to ${newRole.toUpperCase()}?`))
      return;
    ...
    try {
      ...
      toast.info(`User promoted to ${newRole} successfully!`);
    } catch (error) {
      toast.info("Failed to promote user");
    }
  };
  ...
  const UserTable = ({ users, totalCount, isVerified = true, title }) => (
    <div className="bg-white rounded-nonexl shadow-md border border-gray-200 overflow-hidden mb-8">
      <div className="bg-gradient-to-r from-gray-50 to-gray-100 px-6 py-3 border-b border-gray-200">
        <div className="flex items-center justify-between">
          ...
          <span className="bg-blue-100 text-blue-800 text-xs font-medium px-3 py-1 rounded-nonefull">
            {users.length} users
          </span>
        </div>
      </div>
                ...
                >
                  <td className="px-6 py-3">
                    <div className="flex items-center space-x-3">
                      <div
                        className={`w-9 h-9 rounded-nonefull flex items-center justify-center ${
                          isVerified
                      ...
                      <div className="space-y-2 justify-center place-content-center grid">
                        ...
                        >
                          ...
                          <span className="capitalize">
                            {user.role === "development" ? "intern" : user.role}
                          </span>
                        </div>
                        {user.role === "intern" && (
                          <div className="relative group inline-block">
                            <button className="text-xs bg-gray-100 hover:bg-gray-200 px-3 py-1 rounded border text-gray-700">
                              Promote ▾
                            </button>
                  ...
                  {isVerified && (
                    <td className="px-6 py-3">
                      <div className="relative group">
                        ...
                        {/* Hover panel */}
                        <div className="absolute z-20 hidden group-hover:block bg-white shadow-xl border p-3 rounded-lg w-64 mt-2">
                    ...
                    <div className="flex items-center space-x-2">
                      {!isVerified ? (
                        <button
                          ...
                          className="px-3 py-1 bg-green-500 text-white text-xs rounded hover:bg-green-600 transition-colors duration-200 flex items-center space-x-1"
                          title="Accept User"
                        >
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 text-lg">
              Manage users and promote them to admin or HR roles with
              permissions
            </p>
          </div>
          ...
          <div className="bg-white rounded-nonexl shadow-lg p-6 mb-8 border border-gray-200">
            <div className="flex flex-col lg:flex-row lg:items-center gap-4">
              ...
              <div className="text-xs text-gray-600 bg-gray-100 px-4 py-2 rounded-nonelg">
                Showing{" "}
                {filteredVerifiedUsers.length + filteredUnverifiedUsers.length}{" "}
          ...
          {unverifiedUsers.length > 0 && (
            <UserTable
              users={paginatedUnverifiedUsers}
              totalCount={filteredUnverifiedUsers.length}
              isVerified={false}
          ...
          <UserTable
            users={paginatedVerifiedUsers}
            totalCount={filteredVerifiedUsers.length}
            isVerified={true}
            setPage={setVerifiedPage}
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option
                        ...
                        disabled={selectedUser._id === userid && !isAdmin}
                      />
                    </select>
                  </div>
                </div>
                {selectedUser.role !== "user" && (
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {availablePermissions[selectedUser.role]?.map(
                        (permission) => (
                          <div
                            key={permission.id}
