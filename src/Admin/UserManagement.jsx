...
const UserManagement = () => {
    ...
    const handleUpdateUser = async () => {
        ...
        try {
            ...
            await axios.put(`${import.meta.env.VITE_BASE_URL}/update/${selectedUser._id}`, {
                role: selectedUser.role,
                name: selectedUser.name,
                email: selectedUser.email,
                isAdmin: selectedUser.role === "admin",
                mnumber: selectedUser.mnumber,
                permissions: selectedUser?.permissions.map(
                    (permId) =>
                        availablePermissions[selectedUser.role].find((p) => p.id === permId)?.label
                ),
            });
    ...
    const promoteUser = async (userId, newRole) => {
        ...
        const updatedUser = {
            ...userToPromote,
            role: newRole,
            permissions: newRole === "intern" ? [] : availablePermissions[newRole].map((p) => p.id),
        };
        ...
        try {
            await axios.put(`${import.meta.env.VITE_BASE_URL}/update/${userId}`, {
                ...
                permissions: updatedUser?.permissions.map(
                    (permId) =>
                        availablePermissions[updatedUser.role].find((p) => p.id === permId)?.label
                ),
            });
                                    ...
                                    <select
                                        ...
                                        onChange={(e) =>
                                            setSelectedUser({
                                                ...selectedUser,
                                                role: e.target.value,
                                                permissions: e.target.value === "user" ? [] : selectedUser?.permissions,
                                            })
                                        }
                                        className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                    >
